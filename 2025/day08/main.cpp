...
int main() {    
    ...
    while (std::getline(file, line)) {
        ...
        boxes.push_back(b); // add box into vector of boxes
    }
    ...
    for (size_t i = 0; i < boxes.size() - 1; ++i) {
        for (size_t j = i + 1; j < boxes.size(); ++j) {
           ...
           e.u = i;
           e.v = j;
           e.distance = distance(boxes[i], boxes[j]);
           edges.push_back(e);
        }
    }

    // sortirani edgi po velikosti (lahko to nrdi zaradi operator overloadinga)
    std::sort(edges.begin(), edges.end());


    // DELAMO Z INDEKSI!!!!!!!!  (pooint1 in groupe1)
    // groups of conected points
    std::vector<std::vector<size_t>> groups;
    size_t count = 0;
    for (auto it = edges.begin(); it != edges.end() && count < 1000; ++it, ++count) {
        size_t point1 = it->u; //indeks prve točke v paru
        size_t point2 = it->v; // indeks druge točke v paru

        // index grupe v kateri se najhaja indeks točke
        int groupe1 = is_present(point1, groups);
        int groupe2 = is_present(point2, groups);

        if (groupe1 == -1 && groupe2 == -1) {
            groups.push_back({point1, point2}); // nova grupa
        }
        else if (groupe1 != -1 && groupe2 == -1) {
            groups[groupe1].push_back(point2); //gupi ena dodamo index druge točke
        }
        else if (groupe1 == -1 && groupe2 != -1) {
            groups[groupe2].push_back(point1); // grupi 2 dodamo index prve točke
        }
        else if (groupe1 != -1 && groupe2 != -1) {
            if (groupe1 == groupe2) {
                ...
                groups[groupe1].insert(groups[groupe1].end(), groups[groupe2].begin(), groups[groupe2].end());
                groups.erase(groups.begin() + groupe2);
            }
        }
